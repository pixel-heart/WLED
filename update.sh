...
update_to_tag() {
	...
	print_info "Current author's email: ${author_email}"

    print_info "Finding commits authored by ${author_email}..."
    # Find commits authored by the specified author
    local unique_commits
    unique_commits=$(git log --reverse --pretty=format:"%H" --author="${author_email}" HEAD)

    if [ -z "$unique_commits" ]; then
        print_info "No commits by ${author_email} found. Simply resetting to ${tag}"
    else
        local commit_count
        commit_count=$(echo "$unique_commits" | wc -l | tr -d ' ')
        print_info "Found ${commit_count} commits by ${author_email} to preserve:"
    ...
    print_info "1. Record commits authored by ${author_email}"
    print_info "2. Hard reset ${current_branch} to upstream tag ${tag}"
    if [ -n "$unique_commits" ]; then
        print_info "3. Cherry-pick the ${author_email} commits on top"
    fi
    ...
    if [ -n "$unique_commits" ]; then
        ...
        print_info "Successfully cherry-picked all ${author_email} commits!"
    fi
